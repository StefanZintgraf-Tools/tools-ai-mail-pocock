...
Each `Relationship` cell below is a **single enum value** from this vocabulary —
no hybrids (`Conformist / ACL`), no `+`, no free-form qualifier in the cell; a
boundary that genuinely differs by direction is **two rows** (see the
read/write split below), never one hybrid tag (S7).

### Legend

DDD context-mapping vocabulary (Evans, *Domain-Driven Design* Part IV /
Ch. 14) — method vocabulary, kept out of [glossary.md](glossary.md) (which holds
the *product's* language). One line per relationship pattern used as a tag above:
...
| STAFF personal correspondence ↔ ORG-owned mail (at exit/handover) | **Separate Ways**             | Each side owns its own sphere; deliberately not mixed                                                 | Yes — a one-time controlled separation splits personal from the firm's, fair to both (INV11)                               | UC68, UC69, UC70                         |
| MGR ↔ team work-state (oversight)                                 | **Published Language**                       | The team context publishes a *sanitized* work-state vocabulary                                        | Yes — raw actions → work-state (a "board", not a "stopwatch"), dropping anything surveillance-enabling (INV10)             | UC48, UC59                               |
| Delegate/guardian (DLG) ↔ Protected party (PROT)                  | **Customer/Supplier**            | The PROT domain owns its data and sets the ceiling; DLG is downstream, bounded by conferred authority | Yes — permission/scope translation; acts only within INV12's granted authority                                             | UC30, UC72, UC100                        |
...
| Team ↔ Pulled-in collaborator (PIC)                               | **ACL**                            | The team context owns the Thread; PIC gets a minimal view                                             | Yes — redact to "only what they need," isolation restored / revoked on exit (INV11, INV12)                                 | UC56                                     |
| Product ↔ External correspondent (EXT)                            | **Conformist**                                                    | The standard mail/chat world                                                                          | No — ordinary mail/chat semantics adopted wholesale; the only transform is outbound identity masking (INV9), a provenance duty, not a model ACL                                   | UC15, UC43, UC60, UC64, UC92, UC108      |
| Product ↔ EXT's *own* assistant (A2A negotiation)                 | **Conformist**                                                    | The standard plain-mail world (no shared A2A protocol exists yet)                                      | No — each side falls back to plain mail, adopting its semantics wholesale; a shared A2A *Published Language*/Partnership is latent, not built; provenance still governs (INV9) | UC71                                     |
...
| Product ↔ System of record — read (SYS: CRM / sales / support desk)  | **ACL**                                                           | The external system owns its schema; we read as downstream                                            | Yes — translate the external schema into one clean picture                                                                | UC106, UC115, UC73                       |
| Product ↔ System of record — write (SYS: CRM / sales / support desk) | **Customer/Supplier**                                             | The external system owns its schema; we are the downstream customer on write                          | Yes — any committing write-back is translated to the external schema and gated (INV1)                                      | UC106, UC115, UC73                       |
...
| Product ↔ specialist Extension                                    | **Open Host Service**                                             | The *product* publishes a stable extension contract (a Published Language) that extensions conform to                | Yes on the extension's side — it adapts to our published contract; reach bounded by INV12                                  | UC79, UC83                               |
